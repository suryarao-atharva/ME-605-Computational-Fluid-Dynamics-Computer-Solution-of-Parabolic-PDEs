% Explicit Scheme 2D Unsteady heat diffusion

%Defining the domain

domain_size = 80*10^-3;
N = 81;

delta_x = domain_size/(N-1);
delta_y = domain_size/(N-1);
delta_t = 0.05;

%Defining the initial Temperature of the domain

initial_temperature = 300;
T_top = 423;
T_Bottom = 323;
T_Right = 473;
T_ambient = 573;

T = zeros(N,N);
T(:,:) = initial_temperature;
T(1,:) = T_top;
T(N,:) = T_Bottom;
T(:,N) = T_Right;

% disp(T) % gives us the starting temperature of the domain

% Convection Parameter
h = 250;
Lambda = 5;
alpha = 5*10^-6; % Thermal diffusivity

% Initializing the Error and iteration counter;
error = 10;
tolerance = 10^-3;
iterations = 0;


%Temperature along the centerline x = 40, y
T_centerLine_1 = [];


% Check the stability criteria
F = alpha*delta_t/delta_x^2; % Fouriers Number

if F<=0.25
    disp("Stability satisfied")

    % We use the Gauss Siedell Method as our iterative approach
    T_current = zeros(N,N);

    % disp(T_current)

    while error>tolerance

        T_old = T;

        %This loop is for interior nodes
        for i = 2:N-1
            for j = 2:N-1
                T_current(i,j) = T_old(i,j) + F*(T_old(i+1,j)+T_old(i-1,j)+T_old(i,j+1)+T_old(i,j-1)-4*T_old(i,j));
            end
        end

        % This loop is for nodes on Left Boundary
        for k = 2:N-1
            T_current(k,1) = (1/(Lambda + h*delta_x))*(h*delta_x*T_ambient + Lambda*T_old(k,2));
        end

        T_current(1,:) = T_top;
        T_current(N,:) = T_Bottom;
        T_current(:,N) = T_Right;


        %Counting the number of iterations
        iterations = iterations + 1;
        
        %Appending the centerline 1 temperature
        T_centerLine_1 = [T_centerLine_1; T(:, (N+1)/2)'];

        %Calculating the temperature difference in each time step

        Temperature_difference_matrix = T_current-T_old;
        error = norm(Temperature_difference_matrix,2);


        T = T_current;
    end

    % disp(T_centerLine_1) % We have the entire Temperature of a centerline
    [m,n] = size(T_centerLine_1);
    rows_required = round(linspace(1, m, 6));
    T_centerLine_1_subset = T_centerLine_1(rows_required, :); % We take 6 equally spaced intervals
    
    % disp(T_centerLine_1_subset)
    
    %Plotting the temperature along a centerline

    plot(T_centerLine_1_subset.',"LineWidth",2);
    xlabel('Variation in y keeping x = 40');
    ylabel('Temperature');
    title('Temperature variation along centerline (x=40,y)');
    legend('Time 1', 'Time 2', 'Time 3', 'Time 4', 'Time 5', 'Time 6');
    grid on

else
    disp("Stability not satisfied")
end

