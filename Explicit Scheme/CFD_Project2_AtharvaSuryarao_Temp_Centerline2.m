...
if F<=0.25
    ...
    % disp(T_centerLine_2) % We have the entire Temperature of a centerline
    [m,n] = size(T_centerLine_2);
    rows_required = round(linspace(1, m, 6));
    T_centerLine_2_subset = T_centerLine_2(rows_required, :); % We take 6 equally spaced intervals
    
    % disp(T_centerLine_2_subset)
    
    
    plot(T_centerLine_2_subset.',"LineWidth",2);
    xlabel('Variation in x keeping y = 40');
    ylabel('Temperature');
